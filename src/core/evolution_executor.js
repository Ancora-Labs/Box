...
export function shouldHaltOnPreReviewReject(config = {}) {
...
export function repairPrometheusTask(task = {}) {
  const repaired = {
    ...task,
...
export function injectAthenaMissingItems(task, preReview = {}) {
...
function collectAthenaReviewText(preReview = {}) {
...
async function readPrChecks(config, prUrl) {
  ...
  try {
    const raw = await spawnAsync(command, args, { env: process.env, timeoutMs: 60_000 });
 ...
 */
export async function runEvolutionLoop(config, options = {}) {
  ...
  if (!progress || progress.cycle_id !== cycleId) {
    ...
    const escalatedCount = Object.values(progress.tasks).filter(t => t.status === "escalated").length;
  ...
  for (let i = startIndex; i < tasks.length; i++) {
    ...
    while (!taskDone && taskState.attempts < maxAttempts) {
      ...
      let prChecks = { ok: true, passed: true, failed: [], pending: [], total: 0 };
  ...
  // 5. Final summary
  const finalCounts = Object.values(progress.tasks).reduce((acc, t) => {
