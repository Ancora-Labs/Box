...
function ensureValidPremortem(riskLevel, srcPremortem, taskText, targetFiles) {
  ...
  for (const [field, minLen] of [["scenario", 20], ["rollbackPlan", 10]]) {
    if (typeof merged[field] !== "string" || merged[field].trim().length < minLen) {
      merged[field] = scaffold[field];
    }
  }
  return merged;
...
function buildDeterministicRequestBudget(plans = [], executionStrategy = {}) {
